...
public class NexoEconomyExpansion extends PlaceholderExpansion {

    private final EconomyManager economyManager;
    private final DecimalFormat formatter = new DecimalFormat("#,##0.00");

    public NexoEconomyExpansion(EconomyManager economyManager) {
    ...
    public String onPlaceholderRequest(Player player, @NotNull String params) {
        ...
        try {
            currency = NexoAccount.Currency.valueOf(currencyStr.toUpperCase());
        } catch (IllegalArgumentException e) {
            return null; // Si escriben %nexoeconomy_diamantes%, lo ignoramos.
        }

        // Leemos el dinero del caché usando el método nuevo
        BigDecimal balance = economyManager.getBalanceSync(player.getUniqueId(), NexoAccount.AccountType.PLAYER, currency);
