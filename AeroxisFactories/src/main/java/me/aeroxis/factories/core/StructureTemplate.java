...
/**
 * 🏭 AeroxisFactories - Plantilla Estructural (Arquitectura Enterprise Java 21)
 * Rendimiento: Validación espacial O(1) y Blindaje contra Fugas de Mutabilidad.
 */
public class StructureTemplate {
    ...
    public boolean isValid(Block coreBlock) {
        // 🌟 JAVA 21: Inferencia de tipos con 'var' para mayor legibilidad
        for (var entry : requiredBlocks.entrySet()) {
            var rel = entry.getKey();
            var check = coreBlock.getRelative(rel.getBlockX(), rel.getBlockY(), rel.getBlockZ());

            if (check.getType() != entry.getValue()) {
                return false; // Si tan solo un bloque está mal, falla y aborta la iteración
            }
        }
        return true;
    }
    ...
    /**
     * 🌟 PARCHE DE SEGURIDAD: Fuga de Mutabilidad
     * Retorna una vista inmodificable para evitar que otros componentes alteren el plano base.
     */
    public Map<Vector, Material> getRequiredBlocks() { 
        return Collections.unmodifiableMap(requiredBlocks); 
    }
}