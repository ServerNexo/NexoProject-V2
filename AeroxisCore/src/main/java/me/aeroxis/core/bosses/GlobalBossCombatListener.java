...
import me.aeroxis.core.crossplay.CrossplayUtils; // 🌟 IMPORTANTE
...
public class GlobalBossCombatListener implements Listener {
    ...
    @Inject
    public GlobalBossCombatListener(AeroxisCore plugin, CrossplayUtils crossplayUtils) {
        this.plugin = plugin;
    ...
    public void onBossDamage(EntityDamageByEntityEvent event) {
        ...
        // 1. Buscamos si la entidad golpeada es un AeroxisGlobalBoss activo
        AeroxisGlobalBoss boss = AeroxisGlobalBoss.getActiveBoss(victim.getUniqueId());
        if (boss == null) return;
