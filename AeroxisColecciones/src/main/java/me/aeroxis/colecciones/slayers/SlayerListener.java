...
import me.aeroxis.economy.core.EconomyManager; // 🌟 Sinergia Multi-Módulo
...
/**
 * 📚 AeroxisColecciones - Escucha de Combate Slayer (Arquitectura Enterprise)
...
public class SlayerListener implements Listener {
    ...
    // 💉 PILAR 1: Inyección de Dependencias Directa (Adiós AeroxisCore estático)
    @Inject
    public SlayerListener(AeroxisColecciones plugin, SlayerManager slayerManager,
            ...
            if (bossOwner != null) {
                ...
                if (slayer != null && slayer.isBossSpawned()) {
                    ...
                    if (recompensaGemas > 0) {
                        economyManager.updateBalanceAsync(bossOwner.getUniqueId(), AeroxisAccount.AccountType.PLAYER, AeroxisAccount.Currency.GEMS, BigDecimal.valueOf(recompensaGemas), true);
