...
public class NexoViewerMenu {
     ...
     * Renderizado nativo con NexoMenu (cero dependencias externas).
     */
    public void open(Player viewer, Player target, String type) {
        ...
        CrossplayUtils crossplayUtils = JavaPlugin.getPlugin(NexoCore.class).getInjector().getInstance(CrossplayUtils.class);

        // 🌟 Creamos el Menú al vuelo (Clase Anónima)
        NexoMenu gui = new NexoMenu(viewer, crossplayUtils) {
            @Override
