...
public class ArtefactoManager {
    ...
    public ArtefactoManager(NexoItems plugin) {
        this.plugin = plugin;
    ...
    private void registrarEstrategias() {
        ...
        estrategias.put("vara_florbifida", (p, dto) -> {
            ...
            double healAmount = p.getAttribute(Attribute.MAX_HEALTH).getValue() * 0.05;
            ...
            impacto.getWorld().getNearbyEntities(impacto, 4, 4, 4).stream()
                    ...
                    .map(e -> (Player) e)
                    .forEach(ally -> ally.setHealth(Math.min(ally.getAttribute(Attribute.MAX_HEALTH).getValue(), ally.getHealth() + healAmount)));
            return true;
        });
        ...
        estrategias.put("cetro_glacial", (p, dto) -> {
            ...
            p.getNearbyEntities(6, 4, 6).stream()
                    ...
                    .forEach(mob -> {
                        ...
                        Bukkit.getScheduler().runTaskLater(plugin, () -> mob.setAware(true), 60L);
                    });
            return true;
        });
