package me.aeroxis.clans.commands;
...
import me.aeroxis.clans.menu.ClanMenu;
import me.aeroxis.core.crossplay.CrossplayUtils;
import me.aeroxis.core.database.DatabaseManager;
import me.aeroxis.core.user.UserManager;
import me.aeroxis.economy.core.EconomyManager; // Sinergia inyectada del módulo Economy
...
/**
 * 👥 AeroxisClans - Comando Principal de Facciones (Arquitectura Enterprise)
...
public class ComandoClan extends Command {
    ...
    @Inject
    public ComandoClan(AeroxisClans plugin, ClanManager clanManager, UserManager userManager,
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String commandLabel, @NotNull String[] args) {
        ...
        if (user == null) {
            crossplayUtils.sendMessage(player, "&#FF5555[!] Tus datos aún están siendo descargados del Core. Espera un momento.");
            return true;
        }
        ...
        switch (subCommand) {
            ...
            case "kick" -> {
                ...
                Player target = Bukkit.getPlayerExact(args[1]);
                if (target != null) {
                    AeroxisUser tUser = userManager.getUserOrNull(target.getUniqueId());
            ...
            case "deposit" -> {
                ...
                try {
                    ...
                    economyManager.updateBalanceAsync(player.getUniqueId(), AeroxisAccount.AccountType.PLAYER, AeroxisAccount.Currency.COINS, amount, false).thenAccept(success -> {
                        if (success) {
            ...
            case "withdraw" -> {
                ...
                try {
                    ...
                    clanManager.getClanFromCache(user.getClanId()).ifPresent(clan -> {
                        ...
                        economyManager.updateBalanceAsync(player.getUniqueId(), AeroxisAccount.AccountType.PLAYER, AeroxisAccount.Currency.COINS, amount, true);
            ...
            case "tribute", "tributo" -> {
                ...
                clanManager.getClanFromCache(user.getClanId()).ifPresent(clan -> {
                    ...
                    virtualExecutor.submit(() -> {
                        String sql = "UPDATE aeroxis_clans SET monolith_exp = ?, monolith_level = ? WHERE id = CAST(? AS UUID)";
