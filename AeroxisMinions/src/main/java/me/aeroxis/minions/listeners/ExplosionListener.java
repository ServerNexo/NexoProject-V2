...
/**
 * 🤖 AeroxisMinions - Listener de Explosiones (Arquitectura Enterprise)
 * Rendimiento: Llave cacheada O(1), Singleton estricto y escaneo espacial optimizado.
 */
@Singleton
public class ExplosionListener implements Listener {
    ...
    @Inject
    public ExplosionListener(AeroxisMinions plugin) {
        this.plugin = plugin;
        this.displayIdKey = new NamespacedKey(plugin, "minion_display_id"); // 💡 Cacheamos la Key en RAM O(1)
    }
    ...
    private void protegerSueloMinions(List<Block> bloquesDestruidos) {
        // 💡 Código modernizado y ultra rápido: Usamos removeIf nativo y 'var' (Java 21)
        bloquesDestruidos.removeIf(block -> {
            // Calculamos el espacio justo encima del bloque que va a explotar
            var topLoc = block.getLocation().add(0.5, 1.0, 0.5);

            // Escaneamos si hay un Minion encima de este bloque
            for (var entity : topLoc.getWorld().getNearbyEntities(topLoc, 0.5, 0.5, 0.5)) {
