package me.aeroxis.items.artefactos;
...
/**
 * 🎒 AeroxisItems - Manager de Habilidades (Arquitectura Enterprise Java 21)
...
public class ArtefactoManager {
    ...
    @Inject
    public ArtefactoManager(AeroxisItems plugin, UserManager userManager, CrossplayUtils crossplayUtils) {
        this.plugin = plugin;
    ...
    public boolean procesarUso(Player p, ArtefactoDTO dto) {
        ...
        AeroxisUser user = userManager.getUserOrNull(uuid);
        if (user == null) {
            crossplayUtils.sendMessage(p, "&#8b0000[!] Sincronizando datos neuronales. Espera...");
            return false;
        }

        // 💡 NOTA: 'getEnergiaExtraAccesorios' debe existir en AeroxisUser si usas esta lógica.
