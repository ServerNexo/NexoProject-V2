...
/**
 * 🏛️ AeroxisPvP - Listener de Pasivas (Arquitectura Enterprise)
...
public class PasivasListener implements Listener {
    ...
    @Inject
    public PasivasListener(AeroxisPvP plugin, PasivasManager manager, UserManager userManager,
    ...
    public void onCombate(EntityDamageByEntityEvent event) {
        ...
        if (event.getDamager() instanceof Player atacante) {
            AeroxisUser user = userManager.getUserOrNull(atacante.getUniqueId());
        ...
        if (event.getEntity() instanceof Player victima) {
            ...
            AeroxisUser user = userManager.getUserOrNull(id);
            int nivel = user != null ? user.getCombateNivel() : 1;
    ...
    public void onPisadas(PlayerInteractEvent event) {
        if (event.getAction() == Action.PHYSICAL && event.getClickedBlock() != null) {
            AeroxisUser user = userManager.getUserOrNull(event.getPlayer().getUniqueId());
