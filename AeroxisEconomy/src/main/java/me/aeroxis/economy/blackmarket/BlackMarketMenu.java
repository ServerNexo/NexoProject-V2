...
/**
 * 💰 AeroxisEconomy - Menú del Mercado Negro (Arquitectura Enterprise Java 21)
 * Rendimiento: Hilo-Seguro (Async-to-Sync), Dependencias Transitivas, Cero Estáticos y Llaves Cacheadas.
 */
public class BlackMarketMenu extends AeroxisMenu {
    ...
    public BlackMarketMenu(Player player, AeroxisEconomy plugin, BlackMarketManager bmManager, EconomyManager ecoManager, CrossplayUtils crossplayUtils) {
    ...
    public void setMenuItems() {
        ...
        for (int i = 0; i < stock.size() && i < slots.length; i++) {
            ...
            display.editMeta(meta -> {
                String color = bmItem.currency() == AeroxisAccount.Currency.GEMS ? "&#55FF55" : "&#ff00ff";
                String divisaNombre = bmItem.currency() == AeroxisAccount.Currency.GEMS ? "💎 Gemas" : "💧 Maná";
    ...
    public void handleMenu(InventoryClickEvent event) {
        ...
        if (meta.getPersistentDataContainer().has(KEY_BM_INDEX, PersistentDataType.INTEGER)) {
            ...
            if (index >= 0 && index < stock.size()) {
                ...
                ecoManager.updateBalanceAsync(player.getUniqueId(), AeroxisAccount.AccountType.PLAYER, bmItem.currency(), bmItem.price(), false)
                        .thenAccept(success -> {
                            ...
                            player.getScheduler().run(plugin, task -> {
                                if (success) {
                                    ...
                                    String divisa = bmItem.currency() == AeroxisAccount.Currency.GEMS ? "Gemas" : "Maná";
