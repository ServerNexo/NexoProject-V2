...
/**
 * ✨ Menú de Efectos Visuales (Módulo NexoCosmetics)
 * Catálogo Masivo: 6 Alas, 3 Halos, 5 Estelas, 4 Fatalities.
 */
public class VisualEffectsMenu extends NexoMenu {
    ...
    private void renderItem(int slot, String id, Material mat, String name) {
        boolean unlocked = user.getUnlockedCosmetics().contains(id) || player.hasPermission("nexocosmetics.all");
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (selectedId != null && type != null) {
            if (!user.getUnlockedCosmetics().contains(selectedId) && !player.hasPermission("nexocosmetics.all")) {
