package me.aeroxis.chat;

import com.google.inject.Injector;
import me.aeroxis.chat.commands.ComandoChat;
import me.aeroxis.chat.commands.ComandoStream;
import me.aeroxis.chat.database.AeroxisChatDatabase;
import me.aeroxis.chat.utils.AeroxisChatExpansion;
import me.aeroxis.chat.di.ChatModule;
import me.aeroxis.chat.managers.AeroxisAnnouncementManager;
import me.aeroxis.chat.managers.AeroxisChatManager;
import me.aeroxis.chat.managers.AeroxisConnectionListener;
import me.aeroxis.chat.managers.AeroxisDeathListener;
import me.aeroxis.chat.managers.AeroxisLoginListener;
import me.aeroxis.chat.managers.AeroxisPrivateMessageManager;
import me.aeroxis.chat.menu.ChatColorMenu; // 🌟 NUEVO IMPORT: El Menú que mudamos
import me.aeroxis.chat.menu.AeroxisViewerMenu;
import me.aeroxis.chat.utils.ChatPerms;
import me.aeroxis.core.AeroxisCore;
import me.aeroxis.core.crossplay.CrossplayUtils; // 🌟 NUEVO IMPORT
import me.aeroxis.core.menus.CosmeticsHubRegistry; // 🌟 NUEVO IMPORT: El Enchufe del Core
import me.aeroxis.core.user.UserManager; // 🌟 NUEVO IMPORT
import me.aeroxis.core.utils.SoundManager; // 🌟 NUEVO IMPORT
 ...
 */
public class AeroxisChatPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if (core == null) {
            getLogger().severe("❌ Error crítico: Falta AeroxisCore.");
        ...
        var database = childInjector.getInstance(AeroxisChatDatabase.class);
        database.createTables();

        // 🌟 REGISTRO DE MANAGERS Y LISTENERS
        var chatManager = childInjector.getInstance(AeroxisChatManager.class);
        var announcementManager = childInjector.getInstance(AeroxisAnnouncementManager.class);
        var connectionListener = childInjector.getInstance(AeroxisConnectionListener.class);
        var pmManager = childInjector.getInstance(AeroxisPrivateMessageManager.class);
        var deathListener = childInjector.getInstance(AeroxisDeathListener.class);
        var loginListener = childInjector.getInstance(AeroxisLoginListener.class);
        ...
        if (Bukkit.getPluginManager().getPlugin("PlaceholderAPI") != null) {
            new AeroxisChatExpansion(chatManager).register();
        ...
        this.commandHandler.register(childInjector.getInstance(ComandoChat.class));
        this.commandHandler.register(childInjector.getInstance(ComandoStream.class));
        ...
        getServer().getCommandMap().register("nexochat", new Command("nexotienda") {
            ...
            public boolean execute(@NotNull CommandSender sender, @NotNull String label, @NotNull String[] args) {
                ...
                Bukkit.getAsyncScheduler().runDelayed(AeroxisChatPlugin.this, task -> {
        ...
        getServer().getCommandMap().register("nexochat", new Command("nexo_inv") {
            ...
            public boolean execute(@NotNull CommandSender sender, @NotNull String label, @NotNull String[] args) {
                ...
                new AeroxisViewerMenu().open(viewer, target, type);
                return true;
            }
        });
