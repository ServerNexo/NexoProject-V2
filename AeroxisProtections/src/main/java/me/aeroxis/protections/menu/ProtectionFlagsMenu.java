package me.aeroxis.protections.menu;
...
import me.aeroxis.protections.core.ProtectionStone;
...
/**
 * 🛡️ AeroxisProtections - Menú de Leyes del Dominio (Arquitectura Enterprise)
 * Rendimiento: Cero Streams, Llaves Cacheadas O(1), Dependencias Inyectadas y Formato Moderno.
 * Nota: Al ser una GUI transitoria, NO lleva @Singleton.
 */
public class ProtectionFlagsMenu extends AeroxisMenu {

    private final ProtectionStone stone;

    // 🌟 Sinergia inyectada desde la fábrica
    private final AeroxisProtections plugin; // 🌟 FIX: Ahora guardamos el plugin en memoria
    ...
    public ProtectionFlagsMenu(Player player, ProtectionStone stone, AeroxisProtections plugin,
        ...
        // La llave requiere el plugin instance
    ...
    private void createFlagItem(int slot, Material mat, String nombre, String flagId) {
        ...
        if (meta != null) {
            // 🌟 FIX COLOR: Parseo directo con la utilidad de tu Ecosistema, adiós LegacyComponentSerializer
            String nombreFormateado = "&#ff00ff<bold>" + nombre.toUpperCase() + "</bold>";
            meta.displayName(crossplayUtils.parseCrossplay(player, nombreFormateado));
            ...
            for (String line : flagNode.lore()) {
                ...
                lore.add(crossplayUtils.parseCrossplay(player, lineaFormateada));
            }
            meta.lore(lore);
    ...
    public void handleMenu(InventoryClickEvent event) {
        ...
        if (meta.getPersistentDataContainer().has(flagKey, PersistentDataType.STRING)) {
            ...
            // 🛡️ FIX DESACOPLADO: Guardado asíncrono directo al manager inyectado (Cero saltos)
