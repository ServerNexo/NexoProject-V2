package me.aeroxis.war.commands;
...
/**
 * ⚔️ AeroxisWar - Comando Principal (Arquitectura Enterprise)
...
public class ComandoWar {
    ...
    public void challenge(Player player, String targetTag, BigDecimal apuesta) {
        ...
        Optional<AeroxisClan> atacanteOpt = clanManager.getClanFromCache(user.getClanId());
        if (atacanteOpt.isEmpty()) return;
        AeroxisClan atacante = atacanteOpt.get();
                    ...
                    if (rs.next()) {
                        ...
                        clanManager.loadClanAsync(targetId, defensor -> {
                            ...
                            for (Player p : Bukkit.getOnlinePlayers()) {
                                AeroxisUser tu = userManager.getUserOrNull(p.getUniqueId());
